...
Issue [#266](https://github.com/scottlz0310/cloud-migrator/issues/266) の成果物です。`listingData.csv` と同じフォルダーに、CSV が相対パスで参照する画像を配置しています。

## ファイル構成

| ファイル | 用途 | 状態 |
| --- | --- | --- |
| `listingData.csv` | `en-us` / `ja-jp` の Store Listing Data テンプレート | 提出入力用 |
| `store_logo_300x300.png` | 1:1 の Store logo | CSV から参照 |
| `screenshot_dashboard.png` | Dashboard のスクリーンショット | CSV から参照 |
| `screenshot_settings.png` | 設定画面のスクリーンショット | CSV から参照 |
| `screenshot_logs.png` | ログ画面のスクリーンショット | CSV から参照 |
| `app_logo.jpg` | 700x700 の高解像度ロゴ原本 | 派生画像の原本 |
| `store_promo_header.jpg` | プロモーション構図のドラフト | 提出用 CSV では未参照 |

`store_logo_300x300.png` は `app_logo.jpg` を原寸から縮小して作成しています。低解像度画像の拡大は行っていません。スクリーンショットはすべて 1920x1080 です。設定画面などに表示されるアカウント名・パスは提出用のサンプル値です。

## CSV の扱い

`listingData.csv` は Partner Center のエクスポート形式を維持した UTF-8 BOM 付き CRLF の CSV です。`Field`、`ID`、`Type (種類)` の列名や行は削除・改名せず、値だけを更新してください。画像の値は CSV と同じフォルダーを基準にした相対ファイル名です。
...
3. `listingData.csv` と、CSV が参照する 4 画像を含むフォルダーを Partner Center のフォルダーインポートへ渡す。`README.md` は説明用であり、インポート対象ではない。
...
- Store logo は 1:1 の `store_logo_300x300.png` を使用します。原本は `app_logo.jpg` です。
- スクリーンショットは 3 枚を用意しています。Microsoft の掲載ガイダンスでは 1 枚以上が必要で、4 枚以上が推奨されるため、公開前に 4 枚目を追加できる場合は追加します。
- `store_promo_header.jpg` は 1376x768 で、一般的な 1920x1080 の掲載用解像度に達していません。また、現在の文言に実際の転送ルートと再確認が必要な表現があるため、`listingData.csv` から参照していません。提出用プロモーション素材に採用する場合は、高解像度で再作成し、文言とロゴ使用権を確認してから追加します。
