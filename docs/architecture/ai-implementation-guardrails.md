...
寄せ先クラスがまだ存在しない場合でも、実装判断は次の境界名を基準にする。先に理想の境界を明示しておき、各 issue で必要になった境界から薄く実装する。

| 境界 | 置く処理 | 置かない処理 |
|---|---|---|
| Dashboard View | `.razor` component、表示、入力、クリックイベント、軽い表示整形 | provider / pipeline / state DB の選択、設定永続化、転送実行制御 |
| Dashboard ViewModel | 画面状態の集約、転送開始可否、ボタン状態、View から呼ばれる command | 外部 API 直接呼び出し、具象 provider 生成、DB パス決定 |
| Migration Application Service | 転送開始、停止、リセット、実行中 state、`MigrationWork` 相当のユースケース制御 | UI component 固有状態、provider API の詳細実装 |
| Route Descriptor | route 名、source / destination provider key、settings section、state DB key、metrics / phase 表示、wizard step、pipeline key | 実際の API client 生成、転送ループ、UI 表示ロジック |
| Route Runtime Factory | descriptor を受けた provider、pipeline、state DB、metrics reader の組み立て | 画面表示、設定 form の validation 文言、provider 内部 API 分岐 |
| Settings Application Service | route 別の設定表示項目、保存、validation、非表示 provider 設定の保持 | `.razor` への保存処理直書き、他 provider 設定の暗黙削除 |
| Transfer Status Query Service | summary、metrics、phase、latest processing item の route-aware 読み取り | 転送実行、設定変更、provider 生成 |

新規 issue では、変更対象がどの境界に属するかを PR 説明に書く。境界の実体がまだない場合は、次のいずれかを選ぶ。

- 変更範囲内で薄い service / factory / descriptor を作り、その境界へ処理を移す。
- tactical fix として既存箇所へ残し、理由、影響範囲、後続 issue を明記する。

### 既存コードの移行先

現時点のコードには暫定的な責務混在がある。新規変更では、この表を移行先の目安にする。

| 現在の主な場所 | 将来の寄せ先 | 移行方針 |
|---|---|---|
| `App.xaml.cs` の `MigrationWork` | Migration Application Service / Route Runtime Factory | 転送開始ユースケースと provider / pipeline 組み立てを分ける |
| `DashboardPage.razor` の state DB 読み取り | Transfer Status Query Service | summary / metrics / phase / latest item を route-aware query として集約する |
| `SettingsPage.razor` の route 別表示と保存 | Settings Application Service / Route Descriptor | 表示項目と validation を route 定義から導出する |
| wizard の route 選択と step 制御 | Route Descriptor | route ごとの wizard step を単一定義から参照する |
| CLI / Dashboard それぞれの provider 判定 | Route Runtime Factory | provider / pipeline / state DB の選択規則を共有する |
...
- [ ] 変更対象が Target Boundary Design のどの境界に属するか説明されている。
