...
- [x] Dashboard タブが `MudTabs` + 静的 `MudTabPanel` 3 枚で構成される。
- [x] キーボード操作と ARIA 属性が MudBlazor 標準挙動に戻る。
- [x] 概要・詳細情報・ログの表示内容が既存実装から退化しない。
- [x] MUD0002 アナライザーエラーが発生しない。
- [x] Dashboard 関連テストまたはビルド確認を実施する。
