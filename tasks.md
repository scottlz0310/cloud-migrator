...
| 1（並行） | [#161](https://github.com/scottlz0310/cloud-migrator/issues/161) | スライディングウィンドウ指標収集 | 🟡 基盤実装完了（統合は #163） | `SlidingWindowMetrics` 追加・P95/件数&時間切替・設定統合・テスト 24 件 |
| 2 | [#162](https://github.com/scottlz0310/cloud-migrator/issues/162) | AIMD フィードバック制御 + クールダウン | 🟡 評価ロジック実装完了（統合は #163） | `AimdFeedbackController` 追加・4 信号判定・ベースライン EMA・クールダウン・テスト 24 件 |
| 3 | [#163](https://github.com/scottlz0310/cloud-migrator/issues/163) | 並列数補助制御ハイブリッド移行 | 🔜 未着手 | #160 + #162 完了後。**AIMD 評価ロジックは #162 で実装済み。制御ループへの組込みと `WeightedTokenBucket.SetRate()` への反映は本 ISSUE のスコープ。** |
