...
[GitHub Releases](https://github.com/scottlz0310/cloud-migrator/releases) から最新の `CloudMigrator-x.x.x.msi` をダウンロードし、インストーラーを実行します。
...
- 必須設定値
  ...
  - 以下は **環境変数のみ** で設定します（`config.json` には含めない）
...
# DB パスとポートを指定
cloud-migrator dashboard --db "%APPDATA%\CloudMigrator\logs\dropbox_transfer_state.db" --port 8080
...
```bash
cloud-migrator quality-metrics --trx-dir . --coverage-xml coverage.cobertura.xml --output "%APPDATA%\CloudMigrator\logs\quality-metrics.json"
```
...
```bash
cloud-migrator security-scan --project CloudMigrator.slnx --output "%APPDATA%\CloudMigrator\logs\security-scan.json"
```
...
# オプション指定例
cloud-migrator setup bootstrap --config-path "%APPDATA%\CloudMigrator\configs\config.json" --env-path .env
```
...
```bash
cloud-migrator setup doctor
cloud-migrator setup doctor --config-path "%APPDATA%\CloudMigrator\configs\config.json" --strict-dropbox
```
...
```bash
cloud-migrator setup init
cloud-migrator setup init --config-path "%APPDATA%\CloudMigrator\configs\config.json" --env-path .env --force
...
> 上記パスはすべて `%APPDATA%\CloudMigrator\` 配下です。
