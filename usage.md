...
[GitHub Releases](https://github.com/scottlz0310/cloud-migrator/releases) から `cloud-migrator-{version}-win-x64.zip` をダウンロードし、任意のフォルダに展開します。

```powershell
Expand-Archive cloud-migrator-{version}-win-x64.zip -DestinationPath "$env:LOCALAPPDATA\Programs\CloudMigrator"
```

PATH にインストール先を追加します（ユーザー環境変数）:
...
[GitHub Releases](https://github.com/scottlz0310/cloud-migrator/releases) から対象プラットフォームのアーカイブをダウンロードします。

| プラットフォーム | アーカイブ |
|---|---|
| Linux x64 | `cloud-migrator-{version}-linux-x64.tar.gz` |
| macOS x64 | `cloud-migrator-{version}-osx-x64.tar.gz` |
| macOS ARM64（Apple Silicon） | `cloud-migrator-{version}-osx-arm64.tar.gz` |

```bash
mkdir -p ~/.local/bin/cloud-migrator
