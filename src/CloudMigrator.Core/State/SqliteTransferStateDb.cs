...
/// <para>
/// コンストラクタで接続を 1 本開いて保持し、<see cref="DisposeAsync"/> で閉じる。
/// すべての DB アクセスは <see cref="_writeLock"/> で直列化して接続の非スレッドセーフ性に対処する。
/// </para>
/// </summary>
public sealed class SqliteTransferStateDb : ITransferStateDb
{
    ...
    {
        // レコードを一括ロードしてからリーダーを閉じる。
        // Phase D では producer がこのメソッドをストリームしながら consumer が並列に書き込むため、
        // オープンリーダーを長時間保持すると _conn の排他制御と競合する。バッファリングで解消する。
        List<TransferRecord> buffer;

        await _writeLock.WaitAsync(ct).ConfigureAwait(false);
        try
        {
            buffer = new List<TransferRecord>();
            await using var cmd = _conn.CreateCommand();
            cmd.CommandText = """
    ...
    public async Task<TransferDbSummary> GetSummaryAsync(CancellationToken ct)
    {
        ...
        try
        {
            return await GetSummaryInternalAsync(ct).ConfigureAwait(false);
        }
        finally
        {
            _writeLock.Release();
    ...
    private async Task<TransferDbSummary> GetSummaryInternalAsync(CancellationToken ct)
    {
        ...
        // 最近の失敗レコード（最大5件、新しい順）
        await using var failedCmd = _conn.CreateCommand();
        ...
        await using var crawlCmd = _conn.CreateCommand();
    ...
    public async Task ResetAllAsync(CancellationToken ct)
    {
        ...
        try
        {
            ...
            await using var cpCmd = _conn.CreateCommand();
